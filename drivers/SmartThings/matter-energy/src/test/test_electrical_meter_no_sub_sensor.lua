...
-- An Electrical Meter is only required to expose a single endpoint. This covers that minimal
-- composition, where the meter endpoint is the only endpoint reporting energy at all.
...
test.register_coroutine_test(
  "Energy from the meter endpoint is reported when there are no sub-sensor endpoints",
  function()
    ...
    test.socket.matter:__queue_receive({ mock_device.id, clusters.ElectricalEnergyMeasurement.attributes
      .CumulativeEnergyImported:build_test_report_data(mock_device, ELECTRICAL_METER_EP, energy_measurement(100000)) }) --100Wh
    test.socket.capability:__expect_send(
      mock_device:generate_test_message("importedEnergy", capabilities.energyMeter.energy({ value = 100, unit = "Wh" }))
    )
    test.socket.capability:__expect_send(
    ...
    -- export keeps its own 15 minute window, so it reports even though the import one just fired
    test.socket.matter:__queue_receive({ mock_device.id, clusters.ElectricalEnergyMeasurement.attributes
      .CumulativeEnergyExported:build_test_report_data(mock_device, ELECTRICAL_METER_EP, energy_measurement(400000)) }) --400Wh
    test.socket.capability:__expect_send(
